  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
              <span className="badge badge-cyan">
                <Sparkles size={12} /> Green Line 3 • 3 Buses Fleet
              </span>
              <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>45 Seats Per Bus</span>
            </div>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 800 }}>
              {selectedTrip ? `Reserve Seat on ${selectedTrip.busName}` : 'Select Bus on Green Line 3'}
            </h2>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
              {selectedTrip 
                ? `Departure: ${selectedTrip.departureTime} • Pick your seat from the 45-seat cabin map` 
                : 'Bishnandi Ferry Ghat ➔ Araihazar ➔ Gawsia ➔ Green University Campus'}
            </p>
          </div>
        ...
        {/* ========================================================================= */}
        {/* STEP 1: CHOOSE WHICH OF THE 3 BUSES TO BOOK ON GREEN LINE 3 */}
        {/* ========================================================================= */}
        {!selectedTrip && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div className="direction-pill-group" style={{ maxWidth: '420px', width: '100%' }}>
                ...
                >
                  <Navigation size={15} /> To Campus (Bishnandi ➔ Varsity)
                </button>
                <button
                  type="button"
                ...
                >
                  <RotateCcw size={15} /> Return (Varsity ➔ Bishnandi)
                </button>
              </div>
            </div>

            <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <BusIcon size={16} color="var(--gub-cyan)" />
              {direction === 'to_campus' 
                ? 'Select a Bus from Bishnandi Ferry Ghat (3 Buses Operating):' 
                : 'Select a Return Bus from Campus Terminal to Bishnandi:'}
            </div>
              ...
              {currentTrips.map((trip, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '1.4rem',
                      borderLeft: `4px solid ${idx === 0 ? '#06b6d4' : idx === 1 ? '#10b981' : '#8b5cf6'}`,
                      display: 'flex',
                  ...
                  >
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.65rem' }}>
                        <span className={`badge ${idx === 0 ? 'badge-cyan' : idx === 1 ? 'badge-emerald' : 'badge-purple'}`}>
                          {trip.busNumber} • {trip.shiftTitle}
                        </span>
                        <span style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--gub-cyan)' }}>
        ...
        {/* ========================================================================= */}
        {/* STEP 2: 45-SEAT INTERACTIVE MAP & BOOKING FORM FOR CHOSEN BUS ON LINE 3 */}
        {/* ========================================================================= */}
        {selectedTrip && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-input)', padding: '0.85rem 1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)', gap: '0.75rem' }}>
              <div>
                ...
                <div style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                  {selectedTrip.busName} • Departure @ <span style={{ color: 'var(--gub-cyan)' }}>{selectedTrip.departureTime}</span>
                </div>
              </div>

              <span className="badge badge-cyan" style={{ fontSize: '0.82rem', padding: '0.4rem 0.8rem' }}>
              ...
              <form onSubmit={handleConfirmReservation} style={{ display: 'flex', flexDirection: 'column', gap: '1.15rem' }}>
                <div>
                  <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                    <MapPin size={14} color="var(--gub-cyan)" /> Select Boarding Stoppage
                  </label>
                  <select
                ...
                <div style={{ background: 'rgba(6, 182, 212, 0.08)', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid rgba(6, 182, 212, 0.25)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 600 }}>SELECTED SEAT</span>
                    <span style={{ fontSize: '1.4rem', fontWeight: 900, color: selectedSeat ? 'var(--gub-cyan)' : 'var(--text-muted)' }}>
