...
interface GreenLine1ModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const GL1_INBOUND_TRIPS = [
  {
    id: 'gl1-in-1',
    busNumber: 'Bus 01',
...
export const GL1_OUTBOUND_TRIPS = [
  {
    id: 'gl1-out-1',
    busNumber: 'Bus 01',
    busName: 'Green Line 1 (Bus 01)',
...
export const GreenLine1Modal: React.FC<GreenLine1ModalProps> = ({
  isOpen,
  onClose,
  seatBookings,
  onBookSeat,
  ...
  const [selectedTrip, setSelectedTrip] = useState<typeof GL1_INBOUND_TRIPS[0] | null>(null);

  // Form State for Step 2
  const [selectedStoppage, setSelectedStoppage] = useState<string>('Mirpur');
  ...
  const currentTrips = direction === 'to_campus' ? GL1_INBOUND_TRIPS : GL1_OUTBOUND_TRIPS;

  // Filter bookings for a given trip on Line 1
  const getBookingsForTrip = (trip: typeof GL1_INBOUND_TRIPS[0]) => {
      ...
      <div className="booking-terminal-container">
        ...
        {!selectedTrip ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div className="direction-pill-group" style={{ maxWidth: '520px', width: '100%', padding: '5px' }}>
                <button
                  ...
                  style={{
                    background: direction === 'to_campus' ? 'linear-gradient(135deg, #2563eb 0%, #3b82f6 100%)' : undefined,
                    padding: '0.65rem 1.25rem',
                    fontSize: '0.88rem'
                  }}
                >
                  <Navigation size={15} /> To Campus (Mirpur ➔ GUB)
                </button>
                <button
                  type="button"
                  ...
                  style={{
                    background: direction === 'from_campus' ? 'linear-gradient(135deg, #2563eb 0%, #3b82f6 100%)' : undefined,
                    padding: '0.65rem 1.25rem',
                    fontSize: '0.88rem'
                  }}
                >
                  <RotateCcw size={15} /> Return (GUB ➔ Mirpur)
                </button>
              </div>
            </div>
            ...
            <div>
              <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                ...
                {direction === 'to_campus' ? 'Available Buses Departing from Mirpur' : 'Available Return Buses from Campus Terminal'}
              </h3>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                        ...
                        <div style={{
                          ...
                          <span style={{ fontSize: '1.25rem', fontWeight: 900, color: '#60a5fa' }}>{trip.departureTime}</span>
                        </div>
                      </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                          {trip.stoppages.map((s, idx) => (
                            <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.84rem' }}>
                              ...
                              <span style={{ color: '#60a5fa', fontWeight: 700 }}>{s.time}</span>
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          <form onSubmit={handleConfirmReservation} className="booking-grid-layout">
            ...
            <div className="luxury-bus-cabin" style={{ borderColor: 'rgba(59, 130, 246, 0.4)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                <div>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.45rem', margin: 0 }}>
                    <Armchair size={19} color="#3b82f6" /> Select Your Seat (1 to 45)
                  </h3>
                ...
                {selectedSeat && (
                  <span className="badge badge-blue" style={{ fontSize: '0.85rem', padding: '0.35rem 0.75rem', fontWeight: 800 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div className="glass-card" style={{ padding: '1.5rem', borderLeft: '4px solid #3b82f6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <span className="badge badge-blue" style={{ fontSize: '0.72rem' }}>
                      Green Line 1 • {direction === 'to_campus' ? 'Inbound to Campus' : 'Outbound Return'}
                    </span>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: '1.15rem', fontWeight: 900, color: '#60a5fa' }}>{selectedTrip.departureTime}</div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: '1.25rem' }}>
                  <label className="form-label" style={{ fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                    <MapPin size={14} color="#3b82f6" /> Select Your Boarding Stoppage
                  </label>
                  <select
              ...
              <div className="glass-card" style={{ padding: '1.5rem' }}>
                <h4 style={{ fontSize: '1.05rem', fontWeight: 800, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  <User size={16} color="#3b82f6" /> Passenger Details
                </h4>
              ...
              <div className="glass-card" style={{ padding: '1.5rem', background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.12) 0%, rgba(37, 99, 235, 0.05) 100%)', border: '1.5px solid rgba(59, 130, 246, 0.35)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-input)', padding: '0.85rem 1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)', marginBottom: '0.85rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '1.4rem', fontWeight: 900, color: selectedSeat ? '#60a5fa' : 'var(--text-muted)' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 800,
                    background: 'linear-gradient(135deg, #2563eb 0%, #3b82f6 100%)',
                    boxShadow: selectedSeat ? '0 4px 18px rgba(37, 99, 235, 0.5)' : undefined
                  }}
                >
                  {isSubmitting ? (
